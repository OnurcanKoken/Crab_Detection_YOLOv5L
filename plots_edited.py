...
def laser_length(img):
    ...
    height, width, ch = hsv.shape

    # color filters
    # check: BGR numbers of hsv version of the image
    light_red = (0, 100, 200)
    dark_red = (10, 255, 255)
    ...
    # threshold
    th, threshed = cv2.threshold(gray_result, 100, 255, cv2.THRESH_BINARY_INV | cv2.THRESH_OTSU)
    ...
    circles = cv2.HoughCircles(threshed, cv2.HOUGH_GRADIENT, accum_size, minDist,
    ...
    if circles is not None:
        ...
        for ind, i in enumerate(circles[0, :]):
            center = (i[0], i[1])
            # limiting for laser light center
            # for x-axis
            if (i[0] > 1500) or (i[0] < 400):
                continue
            # for y-axis
            if (i[1] > 570) or (i[1] < 485):
                continue
